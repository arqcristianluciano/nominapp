...
export function useAppRoles(): UseAppRolesResult {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (cancelled) return
        const memberRoles = Array.from(new Set((memberData ?? []).map((r: { role: ProjectRole }) => r.role)))
  ...
  return {
    ...
    canViewBankAccounts:
      can('write_bank_accounts') || can('mark_paid') || can('issue_check') || can('write_contractors'),
