  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-app-text">Contratistas</h1>
          <p className="text-sm text-app-muted mt-1">{contractors.length} registrados</p>
        </div>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar contratista..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-app-surface border border-app-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
      </div>

      {loading ? <div className="text-sm text-app-muted">Cargando...</div> : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((c) => (
            <div key={c.id} className="bg-app-surface rounded-xl border border-app-border p-4 hover:border-blue-200 hover:shadow-sm transition-all">
              <div className="flex items-start justify-between">
                <Link to={`/contratistas/${c.id}`} className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-app-muted mt-0.5">{c.specialty || 'Sin especialidad'}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-xs px-2 py-0.5 rounded-full bg-app-chip text-app-muted">{methodLabel[c.payment_method] || c.payment_method}</span>
                      <span className={`w-2 h-2 rounded-full ${c.is_active ? 'bg-green-500' : 'bg-app-subtle'}`} />
                    </div>
                  </div>
                </Link>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => setEditing(c)} className="p-1.5 text-app-subtle hover:text-app-muted">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <Link to={`/contratistas/${c.id}`} className="p-1.5 text-app-subtle hover:text-blue-500">
