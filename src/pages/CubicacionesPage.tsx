    ...
    <div className="space-y-5">
      <div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Cubicaciones</h1>
            <p className="text-sm text-gray-500 mt-0.5">Contrato y avance por contratista</p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <p className="text-xl font-semibold text-gray-900 mt-1">{formatRD(totals.adjusted)}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500">Total avanzado</p>
      ...
      {loading ? (
        <div className="text-sm text-gray-500">Cargando cubicaciones...</div>
      ) : cubicaciones.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-10 text-center">
          <Layers className="w-10 h-10 text-gray-300 mx-auto mb-3" />
          <p className="text-gray-500">No hay cubicaciones registradas</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase">Contratista</th>
                <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase hidden sm:table-cell">Especialidad</th>
                <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase">Contratado</th>
                <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase hidden md:table-cell">Avanzado</th>
                <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase hidden md:table-cell">Restante</th>
                <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase">% Avance</th>
                <th className="w-16" />
              </tr>
            </thead>
              ...
              {cubicaciones.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-3 py-3">
                    <p className="font-medium text-gray-900 text-xs">{c.contractor?.name || '—'}</p>
                  </td>
                  <td className="px-3 py-3 text-gray-500 text-xs hidden sm:table-cell">{c.specialty}</td>
                  <td className="px-3 py-3 text-gray-700 text-xs text-right">{formatRD(c.adjusted_budget || 0)}</td>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2 justify-center">
                      ...
                      <span className="text-[10px] text-gray-600 w-8">{c.completion_percent.toFixed(0)}%</span>
                    </div>
                  </td>
                  <td className="px-2 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => setEditing(c)} className="p-1 text-gray-300 hover:text-blue-500"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDelete(c.id)} className="p-1 text-gray-300 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
