      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{category.code} — {category.name}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3">
          ...
          <div className="relative">
            <label className="text-[10px] font-medium text-gray-500 mb-1 block uppercase tracking-wide">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 border border-gray-200 rounded-lg text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-300"
              />
            </div>
                ...
                {filteredPrices.length === 0 ? (
                  ...
                  filteredPrices.map((p) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-gray-500">{p.unit} · {formatRD(p.unit_price)}</p>
                    </button>
                  ))
                )}
              </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-[10px] font-medium text-gray-500 mb-1 block uppercase tracking-wide">Código</label>
              <input
                type="text"
                placeholder="3.1"
                value={form.code}
                onChange={(e) => set('code', e.target.value)}
                className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-xs focus:ring-1 focus:ring-blue-500"
              />
            </div>
            <div className="col-span-2">
              <label className="text-[10px] font-medium text-gray-500 mb-1 block uppercase tracking-wide">Descripción *</label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-[10px] font-medium text-gray-500 mb-1 block uppercase tracking-wide">Unidad</label>
              <select
                value={form.unit}
                onChange={(e) => set('unit', e.target.value)}
                className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-xs focus:ring-1 focus:ring-blue-500 bg-white"
              >
                {MEASURE_UNITS.map((u) => (
            ...
            <div>
              <label className="text-[10px] font-medium text-gray-500 mb-1 block uppercase tracking-wide">Cantidad *</label>
              <input
                type="number"
                step="any"
                min="0"
            ...
            <div>
              <label className="text-[10px] font-medium text-gray-500 mb-1 block uppercase tracking-wide">Precio unit.</label>
              <input
                type="number"
                step="any"
                min="0"
          ...
          <div>
            <label className="text-[10px] font-medium text-gray-500 mb-1 block uppercase tracking-wide">Notas</label>
            <input
              type="text"
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose} className="px-4 py-2 text-xs text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50">
              Cancelar
            </button>
            <button
              type="submit"
