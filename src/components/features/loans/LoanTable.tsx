        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-app-border">
            {Array.from({ length: loan.installments }, (_, idx) => {
              ...
              return (
                <tr key={installment} className={isPaid ? 'bg-emerald-50/50 dark:bg-emerald-950/10' : 'hover:bg-app-hover'}>
                  ...
                  <td className="text-center px-3 py-1.5">{isPaid ? <span className="text-emerald-600 font-semibold">✓ Pagado</span> : <span className="text-app-subtle">Pendiente</span>}</td>
                </tr>
              )
            })}
          </tbody>
          <tfoot><tr className="bg-app-bg border-t border-app-border">
  ...
  return (
    <>
      <tr className="hover:bg-app-hover transition-colors group">
        <td className="px-4 py-3.5"><div><Link to={`/contratistas/${loan.contractor_id}`} className="font-semibold text-app-text hover:text-blue-600 transition-colors text-sm">{loan.contractor?.name ?? '—'}</Link>{loan.notes && <p className="text-[11px] text-app-subtle mt-0.5">{loan.notes}</p>}</div></td>
        ...
        <td className="px-4 py-3.5 text-center"><span className={`px-2 py-0.5 text-[11px] font-semibold rounded-full ${STATUS_COLOR[loan.status]}`}>{STATUS_LABEL[loan.status]}</span></td>
        <td className="px-2 py-3.5"><div className="flex items-center justify-end gap-1">
          <button onClick={() => setShowAmort((value) => !value)} title="Ver amortización" className="p-1.5 rounded-lg text-app-subtle hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/40 transition-colors">
            {showAmort ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          </button>
          {showActions && (
            <>
              <button onClick={() => onMarkPaid?.(loan.id)} title="Marcar pagado" className="p-1.5 rounded-lg text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 opacity-0 group-hover:opacity-100 transition-all"><CheckCircle className="w-4 h-4" /></button>
              <button onClick={() => onCancel?.(loan.id)} title="Cancelar" className="p-1.5 rounded-lg text-app-subtle hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40 opacity-0 group-hover:opacity-100 transition-all"><XCircle className="w-4 h-4" /></button>
            </>
          )}
        </div></td>
      </tr>
      {showAmort && <tr><td colSpan={8} className="bg-app-bg"><AmortizationTable loan={loan} paid={paid} /></td></tr>}
    </>
  )
}
  ...
  if (loans.length === 0) {
    return (
      <section>
        ...
        <div className="bg-app-surface rounded-xl border border-app-border p-8 text-center"><Banknote className="w-8 h-8 text-app-subtle mx-auto mb-2" /><p className="text-sm text-app-muted">No hay préstamos registrados</p></div>
      </section>
    )
  }
    ...
    <section>
      ...
      <div className="bg-app-surface rounded-xl border border-app-border overflow-hidden shadow-xs">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-app-border">
            {loans.map((loan) => (
              <LoanRow key={loan.id} loan={loan} paid={paidMap[loan.id] ?? 0} onMarkPaid={onMarkPaid} onCancel={onCancel} showActions={showActions} />
            ))}
          </tbody>
        </table>
      </div>
