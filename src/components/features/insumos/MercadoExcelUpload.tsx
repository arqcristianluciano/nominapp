...
export function MercadoExcelUpload({ projectId, hasExisting, onImported }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (parsed.length === 0) {
        setError(
          'No se detectaron líneas válidas. Verifique que el Excel tenga las categorías AJUSTES, EQUIPOS, MANO DE OBRA o MATERIALES como cabeceras de sección.',
        )
        return
      }
      setLines(parsed)
      ...
      <div className="space-y-4">
        {hasExisting && (
          <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg text-xs text-amber-700 dark:text-amber-300">
            ...
            Reemplazará el presupuesto actual. Los contratos creados <strong>no se eliminarán</strong>, pero perderán el
            vínculo con las líneas.
          </div>
        )}
        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-800 rounded-lg p-3 text-xs text-blue-700 dark:text-blue-300 space-y-1">
          ...
          <p>
            Filas de cabecera con el nombre de la categoría:{' '}
            <strong>AJUSTES · EQUIPOS · MANO DE OBRA · MATERIALES</strong>
          </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {CATEGORIES.map((cat) => (
          <div key={cat} className="bg-app-surface border border-app-border rounded-lg p-3 text-xs">
            <span
              className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold mb-1 ${CATEGORY_COLORS[cat]}`}
            >
              {CATEGORY_LABELS[cat]}
            </span>
            <p className="font-semibold text-app-text">
              {formatRD(totals[`total_${cat.toLowerCase().replace('_de_', '_')}` as keyof typeof totals] ?? 0)}
            </p>
      ...
      <div className="bg-app-surface border border-app-border rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-app-border bg-app-bg">
          ...
          <button
            ...
            className="text-xs text-app-subtle hover:text-app-muted flex items-center gap-1"
          >
                  ...
                  return (
                    <tr key={rowKey} className="hover:bg-app-hover">
                      <td className="px-3 py-1.5">
                        <span
                          className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${CATEGORY_COLORS[l.category]}`}
                        >
