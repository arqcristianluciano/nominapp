  ...
  return (
    <div className="bg-app-surface rounded-xl border border-app-border p-3 sm:p-4">
      <div className="flex items-center justify-between mb-2 gap-2">
        ...
        <Link to={`/proyectos/${projectId}/presupuesto`} className="text-xs text-blue-600 hover:text-blue-800 shrink-0">Ver detalle</Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 text-left sm:text-center">
        <div className="flex sm:block items-baseline justify-between gap-2">
          ...
          <p className="text-sm font-semibold text-app-text truncate">{formatRD(totalBudget)}</p>
        </div>
        <div className="flex sm:block items-baseline justify-between gap-2">
          <p className="text-xs text-app-muted">Invertido</p>
          <p className="text-sm font-semibold text-app-text truncate">{formatRD(totalInvested)}</p>
        </div>
        <div className="flex sm:block items-baseline justify-between gap-2">
          <p className="text-xs text-app-muted">Disponible</p>
          <p className={`text-sm font-semibold truncate ${remaining >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatRD(remaining)}</p>
        </div>
      </div>
