...
export function EmptyProjects({ hasSearch, onNew }: { hasSearch: boolean; onNew: () => void }) {
  return (
    <div className="bg-app-surface rounded-xl border border-app-border p-12 text-center">
      ...
      <p className="text-sm text-app-muted mb-5">{hasSearch ? 'Intenta con otro nombre, código o ubicación' : 'Crea tu primer proyecto para empezar a gestionar obras'}</p>
      {!hasSearch && <button onClick={onNew} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition-colors shadow-sm shadow-blue-600/20"><Plus className="w-4 h-4" /> Nuevo proyecto</button>}
    </div>
  )
}
