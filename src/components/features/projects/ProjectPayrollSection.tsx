          ...
          {periods.map((period) => (
            <div key={period.id} className="flex items-center gap-1 sm:gap-2">
              <Link
                ...
                className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 bg-app-surface rounded-xl border border-app-border p-3 sm:p-4 hover:border-blue-300 hover:shadow-sm transition-all"
              >
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <div className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center font-semibold text-sm">{period.period_number}</div>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3 pl-12 sm:pl-0">
                  ...
                  <span className="text-sm font-medium text-app-text whitespace-nowrap">{formatRD(period.grand_total || 0)}</span>
                </div>
              </Link>
