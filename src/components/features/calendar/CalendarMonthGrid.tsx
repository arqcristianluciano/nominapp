  ...
  return (
    <div className="bg-app-surface border border-app-border rounded-xl overflow-hidden">
      <div className="flex items-center justify-between p-4 border-b border-app-border">
        <button onClick={onPrevMonth} className="p-1.5 rounded-lg hover:bg-app-hover text-app-muted">
          <ChevronLeft className="w-4 h-4" />
        </button>
        <h2 className="font-semibold text-app-text">
          {MONTH_NAMES[month]} {year}
        </h2>
        <button onClick={onNextMonth} className="p-1.5 rounded-lg hover:bg-app-hover text-app-muted">
      ...
      <div className="grid grid-cols-7 border-b border-app-border">
        {DAY_NAMES.map((day) => (
          <div key={day} className="py-2 text-center text-[11px] font-semibold text-app-subtle uppercase">
            {day}
          </div>
        ))}
      </div>
          ...
          {Array.from({ length: daysInMonth }).map((_, idx) => {
            ...
            return (
              <button
                ...
                className={`h-20 border-b border-r border-app-border/50 p-1 text-left transition-colors relative ${isSelected ? 'bg-blue-50 dark:bg-blue-950/40' : 'hover:bg-app-hover'}`}
              >
                <span
                  className={`text-xs font-semibold block mb-1 ${isToday ? 'w-5 h-5 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px]' : 'text-app-muted'}`}
                >
                  {day}
                </span>
                <div className="space-y-0.5">
                  {dayEvents.slice(0, 2).map((item) => (
                    <div
                      key={item.id}
                      className={`text-[9px] px-1 py-0.5 rounded truncate font-medium border ${TYPE_COLOR[item.type]}`}
                    >
