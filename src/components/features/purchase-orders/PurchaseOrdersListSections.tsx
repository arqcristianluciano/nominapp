    ...
    <div className="flex items-center justify-between gap-4">
      <div>
        ...
        <p className="text-sm text-app-muted mt-0.5">
          {filteredCount} de {totalCount} solicitud{totalCount !== 1 ? 'es' : ''}
        </p>
      </div>
        ...
        <tbody className="divide-y divide-app-border">
          {requisitions.map((requisition) => (
            <tr key={requisition.id} className="hover:bg-app-hover transition-colors group">
              ...
              <td className="px-4 py-3.5">
                <span className="font-semibold text-app-text text-sm">{requisition.description}</span>
              </td>
              <td className="px-4 py-3.5 text-sm text-app-muted hidden md:table-cell">
                {requisition.project?.name ?? <span className="text-app-subtle">—</span>}
              </td>
  ...
  return (
    <div className="bg-app-surface rounded-xl border border-app-border p-12 text-center">
      ...
      <p className="text-sm text-app-muted mb-5">
        Crea una solicitud y agrega mínimo 2 cotizaciones (o 1 con justificación) para aprobar
      </p>
      {canCreateAnyRequisition && (
        <button
          onClick={onNew}
