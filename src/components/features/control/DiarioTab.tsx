    ...
    <>
      ...
      {txns.loading ? (
        ...
        <div className="bg-app-surface rounded-xl border border-app-border p-8 text-center">
          ...
          <button onClick={onToggleAdd} className="mt-3 text-sm text-blue-600 hover:text-blue-800 font-medium">Registrar la primera transacción</button>
        </div>
      ) : (
        <div className="bg-app-surface rounded-xl border border-app-border overflow-x-auto">
          <table className="w-full min-w-[1200px]">
            ...
            <tfoot><tr className="bg-app-bg border-t border-app-border">
              <td colSpan={6} className="px-2 py-2 text-xs font-semibold text-app-muted text-right">Total ({txns.transactions.length} transacciones):</td>
              <td className="px-2 py-2 text-xs font-bold text-app-text text-right">{formatRD(txns.transactions.reduce((sum, item) => sum + item.total, 0))}</td>
              <td colSpan={6}></td>
            </tr></tfoot>
          </table>
        </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-3 bg-app-surface rounded-xl border border-app-border">
          <span className="text-xs text-app-muted">
            Página {page} de {totalPages} — mostrando {Math.min(pageSize, txns.transactions.length - (page - 1) * pageSize)} de {txns.transactions.length}
          </span>
          <div className="flex items-center gap-1">
            <button onClick={() => onPage(Math.max(1, page - 1))} disabled={page === 1} className="p-1.5 rounded-lg text-app-subtle hover:text-app-muted hover:bg-app-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors"><ChevronLeft className="w-4 h-4" /></button>
            {Array.from({ length: Math.min(5, totalPages) }, (_, idx) => {
              const number = Math.max(1, Math.min(totalPages - 4, page - 2)) + idx
              return (
                <button key={number} onClick={() => onPage(number)} className={`w-7 h-7 text-xs rounded-lg font-medium transition-colors ${number === page ? 'bg-blue-600 text-white' : 'text-app-muted hover:bg-app-hover'}`}>
                  {number}
                </button>
              )
            })}
            <button onClick={() => onPage(Math.min(totalPages, page + 1))} disabled={page === totalPages} className="p-1.5 rounded-lg text-app-subtle hover:text-app-muted hover:bg-app-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      )}
    </>
