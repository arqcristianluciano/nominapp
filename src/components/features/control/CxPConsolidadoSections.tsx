...
export function CxPConsolidadoSummary({
  ...
  return (
    <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-xl p-3 sm:p-4 min-w-0">
      <p className="text-xs text-red-600 dark:text-red-400">{projectFilter === 'all' ? 'Total CxP consolidado' : 'Total CxP (proyecto seleccionado)'}</p>
      <p className="text-lg sm:text-2xl font-bold text-red-700 dark:text-red-400 break-all leading-tight">{formatRD(displayedTotal)}</p>
        ...
        return (
          <div key={group.projectId} className="bg-app-surface rounded-xl border border-app-border overflow-hidden">
            <div className="bg-app-bg px-3 sm:px-4 py-3 border-b border-app-border flex items-start sm:items-center justify-between gap-3">
              ...
              <span className="text-sm font-bold text-red-700 dark:text-red-400 whitespace-nowrap shrink-0 text-right break-all max-w-[40%] sm:max-w-none">{formatRD(group.total)}</span>
            </div>
            ...
            <div className="md:hidden p-3 space-y-3">
              {group.items.map((item) => {
                ...
                return (
                  <div
                    key={rowKey}
                    className="bg-app-bg/40 rounded-lg border border-app-border p-3"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-sm font-medium text-app-text truncate min-w-0 flex-1">{item.supplierName}</p>
                      <p className="text-sm font-semibold text-red-700 dark:text-red-400 whitespace-nowrap shrink-0">{formatRD(item.pending)}</p>
                    </div>
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <tr key={rowKey} className="border-b border-app-border hover:bg-app-hover">
                        <td className="px-4 py-2 text-xs text-app-muted whitespace-nowrap">{new Date(item.date).toLocaleDateString('es-DO')}</td>
                        ...
                        <td className="px-4 py-2 text-xs text-red-700 dark:text-red-400 font-semibold text-right whitespace-nowrap">{formatRD(item.pending)}</td>
