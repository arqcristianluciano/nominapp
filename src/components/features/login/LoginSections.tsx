...
export function LoginBrandPanel() {
  return (
    <div className="hidden lg:flex flex-col justify-between w-[420px] shrink-0 bg-gradient-to-br from-blue-700 via-blue-800 to-indigo-900 p-10 text-white">
      <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-white/20 flex items-center justify-center"><Building2 className="w-5 h-5 text-white" /></div><span className="text-lg font-semibold tracking-tight">NominaAPP</span></div>
      <div className="space-y-8">
        <div><h1 className="text-3xl font-bold leading-snug">Gestión financiera para proyectos de construcción</h1><p className="mt-3 text-blue-200 text-sm leading-relaxed">Control de nóminas, presupuesto, cubicaciones y cuentas por pagar en un solo lugar.</p></div>
        <div className="space-y-4">
          {[{ icon: BarChart3, text: 'Presupuesto vs. real en tiempo real' }, { icon: ShieldCheck, text: 'Aprobaciones con firma digital' }, { icon: Building2, text: 'Multi-proyecto con control financiero' }].map(({ icon: Icon, text }) => <div key={text} className="flex items-center gap-3 text-sm text-blue-100"><div className="w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center shrink-0"><Icon className="w-4 h-4" /></div>{text}</div>)}
        </div>
      </div>
