  ...
  return (
    <div className="grid grid-cols-4 gap-3 mb-8">
      {[
        ...
        <div
          key={label}
          className={`border rounded p-3 ${bold ? 'border-gray-800 bg-gray-800 text-white' : 'border-gray-300'}`}
        >
          <p className={`text-[10px] uppercase tracking-wide ${bold ? 'text-app-subtle' : 'text-app-muted'}`}>
            {label}
          </p>
...
export function PayrollPrintLabor({ laborItems, totalLabor }: { laborItems: LaborLineItem[]; totalLabor: number }) {
  ...
  for (const item of laborItems) {
    ...
    if (!contractorMap.has(contractorId))
      contractorMap.set(contractorId, { name: item.contractor?.name || 'Desconocido', items: [] })
    contractorMap.get(contractorId)!.items.push(item)
  }

  return (
    <section className="mb-8">
      <h2 className="text-sm font-bold uppercase tracking-wider text-app-muted border-b border-gray-300 pb-1 mb-3">
        Mano de Obra
      </h2>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-t border-app-border">
                  ...
                  <td className="px-2 py-1 text-right font-medium text-app-text">
      ...
      <div className="flex justify-end mt-1">
        ...
        <span className="text-xs font-bold text-app-text">{formatRD(totalLabor)}</span>
      </div>
    </section>
  )
}

export function PayrollPrintMaterials({
  materialInvoices,
  totalMaterials,
}: {
  ...
  return (
    <section className="mb-8">
      ...
      <table className="w-full text-xs border border-app-border">
        ...
        <tbody>
          {materialInvoices.map((invoice) => {
            ...
            if (items.length === 0) {
              return (
                <tr key={invoice.id} className="border-t border-app-border">
                  <td className="px-2 py-1 text-app-text align-top">{invoice.supplier?.name || '—'}</td>
                  <td className="px-2 py-1 text-app-muted">{invoice.description}</td>
                  <td className="px-2 py-1 text-app-muted align-top">{invoice.invoice_reference || '—'}</td>
                  <td className="px-2 py-1 text-right font-medium text-app-text">{formatRD(invoice.amount)}</td>
                </tr>
              )
            }
            return items.map((item, idx) => (
              <tr key={item.id} className="border-t border-app-border">
                <td className="px-2 py-1 text-app-text align-top">{idx === 0 ? invoice.supplier?.name || '—' : ''}</td>
                <td className="px-2 py-1 text-app-muted">{item.description}</td>
                <td className="px-2 py-1 text-app-muted align-top">
                  {idx === 0 ? invoice.invoice_reference || '—' : ''}
                </td>
                <td className="px-2 py-1 text-right font-medium text-app-text">{formatRD(item.amount)}</td>
              </tr>
            ))
          })}
        </tbody>
      ...
      <div className="flex justify-end mt-1">
        ...
        <span className="text-xs font-bold text-app-text">{formatRD(totalMaterials)}</span>
      </div>
    </section>
  )
}
    ...
    <section className="mb-8">
      ...
      <table className="w-full text-xs border border-app-border">
        ...
        <tbody>
          {deductions.map((deduction) => (
            <tr key={deduction.id} className="border-t border-app-border">
              <td className="px-2 py-1 text-app-text">{deduction.loan?.contractor?.name ?? '—'}</td>
              <td className="px-2 py-1 text-right font-medium text-red-600">−{formatRD(deduction.amount)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div className="flex justify-end mt-1">
        ...
        <span className="text-xs font-bold text-red-600">
          −{formatRD(deductions.reduce((sum, deduction) => sum + deduction.amount, 0))}
        </span>
      </div>
    </section>
  )
        ...
        <tbody>
          {active.map((cost) => (
            <tr key={cost.id} className="border-t border-app-border">
              ...
              <td className="px-2 py-1 text-right text-app-muted w-16">
                {cost.percentage ? `${cost.percentage}%` : 'Fijo'}
              </td>
              <td className="px-2 py-1 text-right font-medium text-app-text w-24">
    ...
    <>
      <div className="border-t-2 border-gray-800 pt-4 flex justify-end">
        <div className="text-right">
          ...
          <p className="text-2xl font-bold text-app-text">{formatRD(grandTotal)}</p>
        </div>
      </div>
