...
export function ContractorsHeader({
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div>
        <h1 className="text-2xl font-bold text-app-text">{t('contractors.title')}</h1>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-sm text-app-muted">{t('contractors.registered', { count: total })}</span>
          {active > 0 && (
            <span className="px-2 py-0.5 text-[11px] font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 rounded-full">
              {t('contractors.active', { count: active })}
            </span>
          )}
        </div>
      </div>
...
export function ContractorsSearch({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const { t } = useTranslation()
  return (
    <div className="relative max-w-sm">
      ...
      <input
        type="text"
        placeholder={t('contractors.search_placeholder')}
        value={value}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {contractors.map((contractor) => {
        const parent = contractor.parent_contractor_id
          ? contractorsById.get(contractor.parent_contractor_id)
          : undefined
        const isSub = !!contractor.parent_contractor_id
        return (
          <div
            key={contractor.id}
            className={`group bg-app-surface rounded-xl border border-app-border p-4 hover:border-amber-300 dark:hover:border-amber-700 hover:shadow-sm transition-all ${isSub ? 'md:ml-6 border-l-4 border-l-amber-400 dark:border-l-amber-600' : ''}`}
          >
            <div className="flex items-start gap-3">
              <Link to={`/contratistas/${contractor.id}`} className="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-app-muted mt-0.5 truncate">
                    {contractor.specialty || t('contractors.no_specialty')}
                  </p>
                  {isSub && (
                    <div className="mt-1">
                      <span
                        className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300"
                        title={t('contractors.sub_contractor')}
                      >
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-app-chip text-app-muted font-medium">
                      {METHOD_LABEL[contractor.payment_method] || contractor.payment_method}
                    </span>
                    <span
                      className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${contractor.is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300' : 'bg-app-chip text-app-subtle'}`}
                    >
                      {contractor.is_active ? t('contractors.status_active') : t('contractors.status_inactive')}
                    </span>
                  </div>
                </div>
              </Link>
              <div className="flex flex-col items-center gap-1 shrink-0">
                ...
                <Link
                  ...
                  title={t('contractors.view_detail')}
                >
...
export function EmptyContractorsState({ hasSearch, onNew }: { hasSearch: boolean; onNew: () => void }) {
  const { t } = useTranslation()
  const { canWriteContractors } = useAppRoles()
  return (
    <div className="bg-app-surface rounded-xl border border-app-border p-12 text-center">
      ...
      <p className="text-base font-semibold text-app-text mb-1">
        {hasSearch ? t('contractors.empty_no_results') : t('contractors.empty_no_contractors')}
      </p>
      <p className="text-sm text-app-muted mb-5">
        {hasSearch ? t('contractors.empty_try_other') : t('contractors.empty_register')}
      </p>
      {!hasSearch && canWriteContractors && (
        <button
          onClick={onNew}
        ...
        >
          <Plus className="w-4 h-4" /> {t('contractors.new_contractor')}
        </button>
      )}
    </div>
  )
