  ...
  return (
    <div className="bg-app-surface rounded-xl border border-app-border overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-app-border">
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">
          <Plus className="w-3.5 h-3.5" />Agregar
        </button>
      </div>

      {showForm && <DocFormPanel form={form} setForm={setForm} onCancel={() => setShowForm(false)} onSave={handleSave} saving={saving} />}
...
function DocFormPanel({ form, setForm, onCancel, onSave, saving }: {
  form: DocForm
  ...
  const inputClass = 'w-full px-2 py-1.5 text-xs border border-app-border rounded-lg bg-app-bg text-app-text focus:outline-none focus:ring-1 focus:ring-blue-500'
  return (
    <div className="px-4 py-3 border-b border-app-border bg-app-hover/30 space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="sm:col-span-2">
          ...
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ej: Cédula de identidad" className={inputClass} />
        </div>
        <div>
          <label className="text-xs text-app-muted block mb-1">Vencimiento</label>
          <input type="date" value={form.expiry_date ?? ''} onChange={(e) => setForm({ ...form, expiry_date: e.target.value || null })} className={inputClass} />
        </div>
      </div>
      ...
      <tbody className="divide-y divide-app-border">
        {docs.map((doc) => (
          <tr key={doc.id} className="hover:bg-app-hover/50">
            ...
            <td className="px-4 py-2.5 text-center"><DocStatusBadge status={doc.status} /></td>
            <td className="px-4 py-2.5 text-app-muted text-xs hidden md:table-cell">{doc.expiry_date ?? 'Sin vencimiento'}</td>
            <td className="px-4 py-2.5">
              <button onClick={() => onDelete(doc.id)} className="p-1.5 text-app-subtle hover:text-red-500 rounded hover:bg-red-50 dark:hover:bg-red-950/30">
...
function DocStatusBadge({ status }: { status: ContractorDocument['status'] }) {
  if (status === 'valid') return <span className="inline-flex items-center gap-1 text-green-600 text-xs"><CheckCircle className="w-3.5 h-3.5" />Vigente</span>
  if (status === 'expiring') return <span className="inline-flex items-center gap-1 text-yellow-600 text-xs"><Clock className="w-3.5 h-3.5" />Por vencer</span>
  if (status === 'expired') return <span className="inline-flex items-center gap-1 text-red-600 text-xs font-semibold"><AlertTriangle className="w-3.5 h-3.5" />Vencido</span>
  return <span className="inline-flex items-center gap-1 text-app-muted text-xs"><AlertTriangle className="w-3.5 h-3.5" />Faltante</span>
}
