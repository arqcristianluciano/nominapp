...
function MaterialInvoiceMobileCardComponent({ invoice, canEdit, onDelete }: MaterialInvoiceRowProps) {
  return (
...
function MaterialInvoiceRowComponent({ invoice, canEdit, onDelete }: MaterialInvoiceRowProps) {
  return (
...
export function MaterialInvoicesSection({ invoices, canEdit, total, onOpenAdd, onDelete }: Props) {
  return (
    <section>
        ...
        <>
          ...
          <ul className="sm:hidden space-y-2">
            {invoices.map((invoice) => (
              <MaterialInvoiceMobileCard key={invoice.id} invoice={invoice} canEdit={canEdit} onDelete={onDelete} />
            ))}
          </ul>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[520px]">
                ...
                <tbody className="divide-y divide-app-border">
                  {invoices.map((invoice) => (
                    <MaterialInvoiceRow key={invoice.id} invoice={invoice} canEdit={canEdit} onDelete={onDelete} />
                  ))}
                </tbody>
              </table>
            </div>
