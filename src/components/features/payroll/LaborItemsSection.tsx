...
function LaborItemMobileCardComponent({ item, isDraft, onEdit, onDelete }: LaborItemRowProps) {
  return (
    <li className="bg-app-surface rounded-xl border border-app-border p-3">
      <div className="flex items-start justify-between gap-2">
        ...
        {isDraft && (
          <div className="shrink-0 -mr-2 -mt-2 flex items-center">
            <button
              onClick={() => onEdit(item)}
...
function LaborItemRowComponent({ item, isDraft, onEdit, onDelete }: LaborItemRowProps) {
  return (
...
export function LaborItemsSection({ items, isDraft, total, onOpenAdd, onEdit, onDelete }: Props) {
  return (
    <section>
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          <ul className="sm:hidden space-y-2">
            {items.map((item) => (
              <LaborItemMobileCard key={item.id} item={item} isDraft={isDraft} onEdit={onEdit} onDelete={onDelete} />
            ))}
          </ul>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[640px]">
                ...
                <tbody className="divide-y divide-app-border">
                  {items.map((item) => (
                    <LaborItemRow key={item.id} item={item} isDraft={isDraft} onEdit={onEdit} onDelete={onDelete} />
                  ))}
                </tbody>
              </table>
            </div>
