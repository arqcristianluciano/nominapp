...
export function IndirectCostsSection({
  ...
  return (
    <section>
          ...
          <tbody className="divide-y divide-app-border">
            {costs.map((cost) => (
              <tr key={cost.id} className={cost.is_active ? '' : 'opacity-50'}>
                <td className="px-4 py-2.5 w-10"><input type="checkbox" checked={cost.is_active} disabled={!isDraft || saving} onChange={(e) => onToggleActive(cost.id, e.target.checked)} className="w-4 h-4 accent-blue-600 cursor-pointer disabled:cursor-not-allowed" /></td>
      ...
      <div className="mt-3 bg-purple-50 rounded-lg px-4 py-3 flex justify-between items-center"><span className="text-sm font-medium text-purple-900">Total indirectos</span><span className="text-sm font-semibold text-purple-900">{formatRD(total)}</span></div>
    </section>
  )
}
