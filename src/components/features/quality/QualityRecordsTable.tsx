            ...
            <div key={record.id} className="bg-app-surface rounded-xl border border-app-border p-3 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-app-muted mt-0.5">Colada: {new Date(record.pour_date).toLocaleDateString('es-DO')}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  ...
                  <p className={`${daysLeft !== null && daysLeft <= 3 && daysLeft >= 0 ? 'text-amber-600 font-medium' : daysLeft !== null && daysLeft < 0 ? 'text-red-600 font-medium' : 'text-app-text'}`}>
                    {expectedDate ? (
                    ...
                    {daysLeft !== null && daysLeft <= 7 && expectedDate && (
                      <span className="block text-[10px]">({daysLeft < 0 ? `${Math.abs(daysLeft)}d vencido` : `en ${daysLeft}d`})</span>
                    )}
                  </p>
                </div>
                <div>
                  <p className="text-app-muted">Esperada</p>
                  <p className="text-app-text">{record.expected_resistance ? `${record.expected_resistance} kg/cm²` : '—'}</p>
                </div>
                <div>
                  <p className="text-app-muted">Real</p>
                  <p className={`font-semibold ${record.status === 'failed' ? 'text-red-600' : record.status === 'passed' ? 'text-green-600' : 'text-app-subtle'}`}>
              ...
              <div className="flex items-center justify-end gap-1 pt-2 border-t border-app-border">
                <button onClick={() => onEdit(record)} aria-label="Editar ensayo" className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-app-subtle hover:text-blue-500">
                  <Pencil className="w-4 h-4" />
                </button>
                <button onClick={() => onDelete(record.id)} aria-label="Eliminar ensayo" className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-app-subtle hover:text-red-500">
            ...
            {records.map((record) => {
              ...
              return (
                <tr key={record.id} className="hover:bg-app-hover">
                  ...
                  <td className="px-2 py-2.5"><div className="flex items-center gap-1"><button onClick={() => onEdit(record)} aria-label="Editar ensayo" className="p-2 min-h-[36px] min-w-[36px] flex items-center justify-center text-app-subtle hover:text-blue-500"><Pencil className="w-3.5 h-3.5" /></button><button onClick={() => onDelete(record.id)} aria-label="Eliminar ensayo" className="p-2 min-h-[36px] min-w-[36px] flex items-center justify-center text-app-subtle hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button></div></td>
                </tr>
              )
            })}
          </tbody>
